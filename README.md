...
pages/
  ...
  ProductManagement.tsx     ★ 송장 양식 · 발주처 · 제품 그룹 · 제품(SKU) 관리
...
### 3. 제품 그룹 (product_groups)
같은 발주처·같은 송장 양식으로 나가는 제품들의 **공통 설정 묶음**입니다.
그룹에 송장 양식 · 발주처 · 택배비 · 기타비용 · 수수료율 · 과세구분 · 묶음배송을
한 번 넣어두면, 새 제품은 **그룹만 고르고 판매가·매입가만 입력**하면 됩니다.
송장 양식은 그룹이 쓰는 것을 그대로 쓰므로 엑셀을 다시 등록할 필요가 없습니다.

그룹은 **입력 기본값**입니다. 저장되는 순간 값은 제품 행에 복사되고,
송장 출력·정산은 제품 값만 봅니다. 그래서

- 제품 하나만 택배비가 다르면 그 제품에서 고쳐도 그룹은 그대로이고,
- 그룹을 지워도 제품은 값을 들고 남아 송장이 그대로 나옵니다.

그룹 설정을 바꾼 뒤 소속 제품에 다시 입히려면 그룹 목록의 **'설정 일괄 적용'**을
누릅니다 (판매가·매입가는 건드리지 않습니다).
