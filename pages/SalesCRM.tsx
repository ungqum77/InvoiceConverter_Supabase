...
import { Calendar, TrendingUp, Filter, Download, PieChart, BarChart2, RefreshCw } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 min-h-screen">
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                    <span className="font-bold text-sm text-slate-700">상세 거래 내역</span>
                    <span className="text-xs text-slate-400 font-medium">총 {filteredRecords.length}건</span>
                </div>
                    ...
                    <table className="w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredRecords.length === 0 ? (
                                <tr><td colSpan={7} className="p-12 text-center text-slate-400">해당 기간의 데이터가 없습니다.</td></tr>
                            ) : (
                        ...
                        {filteredRecords.length > 0 && (
                            <tfoot className="sticky bottom-0 z-10 bg-slate-100 font-black text-slate-800 border-t-2 border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                                <tr>
                                    <td colSpan={3} className="px-4 py-3 text-center">합 계</td>
